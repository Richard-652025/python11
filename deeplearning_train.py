...
def evaluate_regression_model(model,dataloader,device,criterion):
    ...
    with torch.no_grad():
        for inputs,targets in dataloader: # 每一轮遍历一个批
            ...
            loss=criterion(outputs,targets) # 每一批计算一个损失

            running_loss+=loss.item()

    return running_loss/len(dataloader) 
    # len(dataloader)返回数据集有多少个批
    # len(dataloader.dataset)返回数据集的样本总数
    ...
    with tqdm(total=num_epochs * len(train_loader)) as pbar:
        for epoch_id in range(num_epochs):
            ...
            for datas, labels in train_loader:
                ...
                loss_value = loss.to(device).item()
...
def plot_learning_loss_curves(record_dict,sample_step=500):
    ...
    ax.set_xticklabels(map(lambda x: f"{int(x / 1000)}k", x_data))
    ax.set_xlabel("step")
    ax.set_ylabel("loss")
    ...
    with tqdm(total=num_epochs*len(train_loader),desc='train progress') as pbar:
        for epoch_id in range(num_epochs):
            ...
            for datas,labels in train_loader:
                ...
                deep_mean=torch.mean(deep,dim=1) # 沿着第一维求平均
                # print(f"deep_mean.shape: {deep_mean.shape}")
                ...
                if global_step % eval_step == 0:
                    epoch_val_loss = evaluate_multi_output_regression_model(
                        model,
                        val_loader,
                        device,
                        criterion)
